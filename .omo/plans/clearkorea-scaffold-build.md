...
- Completed through Wave 5 (Tasks 1-15 including 2A/2B/7A/7B). Only Wave 6 (Task 16) full-surface verification remains.
...
- Task 10 completed in commit `1d5344a feat(stations): port affected polling board`; `/app/stations` ballot-box SVG 3/4/6-col grid, 가나다 sort, severity filters, summary, disclaimer, Today/Rallies entry links, review-only Cron refresh draft. Verified: lint/typecheck/test, build, server-render + browser filter QA.
- Task 11 completed in commit `ac99802 feat(admin): add queues and role-gated settings`; noindex role-gated `/admin` (access-denied for non-admins), admin/super action gating, super-only auto-hide settings, audit entry on every action, deferred review routes. Verified: 403 route gating, noindex, 23 unit tests.
- Task 12 completed in commit `f93db15 feat(security): add rate limits and cache guardrails`; in-memory rate limiter (no Redis) + Turnstile abstraction wired into voices/tips after the auth gate, cache headers on read endpoints, Cloudflare/abuse runbook. Verified: auth-before-rate-limit ordering, cache headers, 13 unit tests.
- Task 13 completed in commit `c1fa57e feat(moderation): add AI-assisted review queues`; hot-entry-once `ai_checked`, heuristic/OpenAI classifier abstraction, soft-hide + popular/report queues, trust scoring, conservative auto-hide, recommend-only triage agents + anomaly hook, admin-gated hot-check route. Verified: admin-gate 403, 22 unit tests.
- Task 14 completed in commit `e81a673 feat(observability): add PostHog flags and masking`; env-gated PostHog provider (no-op without key), `data-ph-mask` on composer + voice body, feature flags (hot weights/threshold/kill switches), PII-safe error capture, setup guide, no Sentry. Verified: masking in HTML, provider no-op, 12 unit tests.
- Task 15 completed in commit `db8bf16 ci(deploy): add deployment checks and env hygiene`; CI workflow (lint/typecheck/test/build/feed), deployment runbook, AGPL footer + metadata. Verified: 244 tests, build, clean secret scan (legacy email names only in forbidden-policy/guard context).
- **Next wave to execute: Wave 6 — Task 16** (full-surface verification, launch auth regression, secret scan, accessibility, responsive screenshots, cleanup) plus the Final Verification Wave F1-F4.
