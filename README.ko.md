...
- 출시 환경의 참여는 카카오 또는 네이버 계정 연동이 필요합니다. 투명한 목소리를 남기되, 최소한의 신원 책임성을 유지하기 위한 기준입니다.
- 개발/테스트 우회는 비프로덕션 QA 용도로만 둘 수 있으며 launch mode에서는 비활성화합니다.
...
- 카카오/네이버로 연동한 참여자에게 변경 불가 자동 닉네임이 발급됩니다.
...
- 시크릿, 어드민 부트스트랩 식별자, API key, DB password, 배포 토큰은 절대 커밋하지 않습니다.
- Supabase, Vercel, Cloudflare, PostHog는 무작정 직접 연동하지 않습니다. 호스팅 프로젝트, DNS, 결제, OAuth secret, WAF rule, 분석 프로젝트를 바꾸는 critical setup은 user approval을 받아야 하며, critical하지 않은 작업은 먼저 추상화하고 마지막에 final setup guide로 연결을 돕습니다.
